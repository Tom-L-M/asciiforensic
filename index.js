// This is a port of ASCIIFORENSIC.py from D4RKW4RE
// https://github.com/D4RKW4R3/D4RKW4RE/tree/main
...
function analysis (data = '', colorize = false) {
    const isAbnormalChar = code => (code < 31 || code > 128) && (![7,8,9,10,11,12,13,27].includes(code));
    ...
    for (let char of data) {
        ...
        if (colorize && isAbnormalChar(code)) {
            acc += `${colors[0]}${char}\t${code}\t${code.toString(16)}\tU+${char.codePointAt(0).toString(16).padStart(4,'0').toUpperCase()}${colors[1]}\n`;
        }

        else {
            acc += `${char}\t${code}\t${code.toString(16)}\tU+${char.codePointAt(0).toString(16).padStart(4,'0').toUpperCase()}\n`;
        }
    }
...
(function main () {
    ...
    switch (mode) {
        case '-h': case '--help':       console.log(banner);      break;
        case '-t': case '--table':      printASCIITable();        break;
        case '-s': case '--scan':       analysis(data, false);    break;
        case '-a': case '--autoscan':   analysis(data, true);     break;
        default: console.log(`<> Error: Invalid mode [${mode}]`); break;
    }
